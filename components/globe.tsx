...
function createLabel(text: string, position: THREE.Vector3, onClick?: () => void) {
  ...
  label.element.style.pointerEvents = "auto"; // clickable
  return label;
}
    ...
    const app = {
      async initScene(scene: THREE.Scene, camera: THREE.Camera, renderer: THREE.WebGLRenderer) {
        try {
          ...
          labelRenderer.domElement.style.pointerEvents = "none"; // allow interaction with canvas
          ...
          function animate() {
            ...
            lineArrows.forEach((la) => {
              ...
              const tangentOnSphere = tangent
                .clone()
                .sub(normal.multiplyScalar(tangent.dot(normal)))
                .normalize();
